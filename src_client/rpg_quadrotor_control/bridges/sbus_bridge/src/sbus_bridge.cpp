...
SBusBridge::SBusBridge(const ros::NodeHandle& nh, const ros::NodeHandle& pnh)
  ...
  control_sub_ = nh_.subscribe(
      "control", 1, &SBusBridge::controlCallback, this);
...
void SBusBridge::controlCallback(const geometry_msgs::PoseStamped::ConstPtr& msg) {
  std::lock_guard<std::mutex> main_lock(main_mutex_);
  ...
  ROS_INFO("[%f 'thr' / %f 'roll' / %f 'pit'/ %f 'yaw'] ", msg->pose.orientation.w, 
                                msg->pose.position.x, 
                                msg->pose.position.y, 
                                msg->pose.position.z);
  ...
  SBusMsg sbus_msg_to_send;
  {
    std::lock_guard<std::mutex> battery_lock(battery_voltage_mutex_);
    // Set commands
    sbus_msg_to_send = generateSBusMessage(msg->pose.orientation.w, 
                                           msg->pose.position.x, 
                                           msg->pose.position.y, 
                                           msg->pose.position.z);
    // Battery voltage mutex is unlocked because it goes out of scope here
...
SBusMsg SBusBridge::generateSBusMessage(float throttle, 
                                        float roll, 
                                        float pitch, 
                                        float yaw) const {
  SBusMsg sbus_msg;
  ...
  sbus_msg.setThrottleCommand(round(throttle * (SBusMsg::kMaxCmd - SBusMsg::kMeanCmd) + SBusMsg::kMeanCmd));
  sbus_msg.setRollCommand(round(roll * (SBusMsg::kMaxCmd - SBusMsg::kMeanCmd) + SBusMsg::kMeanCmd));
  sbus_msg.setPitchCommand(round(pitch * (SBusMsg::kMaxCmd - SBusMsg::kMeanCmd) + SBusMsg::kMeanCmd));
  sbus_msg.setYawCommand(round(yaw * (SBusMsg::kMaxCmd - SBusMsg::kMeanCmd) + SBusMsg::kMeanCmd));
