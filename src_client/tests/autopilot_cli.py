...
class AutopilotWidget:
    ...
    def connect(self, quad_namespace):
        ...
        self._arm_bridge_pub = rospy.Publisher(
            quad_namespace+'/sbus_bridge/arm', std_msgs.Bool, queue_size=1)
        ...
        self._go_to_pose_pub = rospy.Publisher(
            ...
            queue_size=1)
        self._send_control = rospy.Publisher(
            quad_namespace+'/control', geometry_msgs.PoseStamped,
            queue_size=1)
    ...
    def on_publish_control(self, throttle, roll, pitch, yaw):
        try:
            go_to_pose_msg = geometry_msgs.PoseStamped()
            go_to_pose_msg.pose.position.x = float(roll)
            go_to_pose_msg.pose.position.y = float(pitch)
            go_to_pose_msg.pose.position.z = float(yaw)

            go_to_pose_msg.pose.orientation.w = float(throttle)
            # go_to_pose_msg.pose.orientation.z = math.sin(heading / 2.0)

            self._send_control.publish(go_to_pose_msg)
        except:
    ...
    def on_button_disarm_bridge_clicked(self):
